...
ssize_t emoticon_write(struct file *filp, char *buf, size_t count, loff_t *f_pos);
...
ssize_t emoticon_read(struct file *filp, char *buf, 
  ...
  printk("<1>emoticon_read: count = %d, available_to_read = %d\n", count, available_to_read);
...
ssize_t emoticon_write(struct file *filp, char *buf, 
                    size_t count, loff_t *f_pos) { 
 
  char data[80];
			    
  printk("<1>emoticon_write: count = %d\n", count);
